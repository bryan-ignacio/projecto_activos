...
int main() {
    ...
    Usuario *u1 = new Usuario("user1", "user1", "Bryan Ignacio", "Guatemala", "IGGS");
    Node *n1 = new Node(u1);
    matriz->insertarValor(n1, n1->user->getDepartamento(), n1->user->getEmpresa());

    Usuario *u2 = new Usuario("user2", "user2", "Bryan Ignacio", "Jutiapa", "IGGS");
    Node *n2 = new Node(u2);
    matriz->insertarValor(n2, n2->user->getDepartamento(), n2->user->getEmpresa());

    Usuario *u3 = new Usuario("user3", "user3", "Bryan Ignacio", "Guatemala", "Cinepolis");
    Node *n3 = new Node(u3);
    matriz->insertarValor(n3, n3->user->getDepartamento(), n3->user->getEmpresa());

    Usuario *u4 = new Usuario("user4", "user4", "Bryan Ignacio", "Escuintla", "Max");
    Node *n4 = new Node(u4);
